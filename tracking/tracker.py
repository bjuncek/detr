...
class Tracker:
    ...
    def __init__(self, obj_detect, reid_network, tracker_cfg):
        ...
        self.public_detections = True
        self.inactive_patience = 50
        self.do_reid = tracker_cfg['do_reid']
        self.max_features_num = 10
        self.reid_sim_threshold = tracker_cfg['reid_sim_threshold']
        self.reid_iou_threshold = tracker_cfg['reid_iou_threshold']
        self.do_align = True
        self.motion_model_cfg = {'enabled': False, 'n_steps': 5, 'center_only': False}

        self.warp_mode = getattr(cv2, 'MOTION_EUCLIDEAN')
    ...
    def regress_tracks(self, blob):
        ...
        # regress
        boxes, scores = self.obj_detect.predict_boxes(pos) #FIXME: bruno modify this
    ...
    def step(self, blob):
        ...
        ### Blob contains
        # img: tensor
        # dets: num_dets*4 tensor
        # img_path
        # gt: dict{id: list of coordinates}
        # vis: doct{some_umber: 1x1 tensor score}

        # at first step this hsould be skipped as nothign really changes
        for t in self.tracks:
        ...
        # first, put the image into the model wrapper
        self.obj_detect.load_image(blob['img'])

        # if using public detections, use them to regress the boxes
        ...
        # else run the detector on the image
        # THIS IS DISABLED IN THE DETR VARIANT
        else:
            print("I never use this, no?")
            boxes, scores = self.obj_detect.detect(blob['img'])
        

        # TODO: Are the boxes in the correct scale (investigate)
        if boxes.nelement() > 0:
        ...
        for t in self.tracks:
            if t.id not in self.results.keys():
                self.results[t.id] = {}
            tpos = t.pos[0].cpu()
            self.results[t.id][self.im_index] = np.concatenate([tpos.numpy(), np.array([t.score.cpu().numpy()])])
